...
LEDGER_MIGRATION = next(
    path for path in MIGRATIONS if path.stem == "007_add_schema_migration_ledger"
)
...
def _verify(connection) -> dict[str, str]:
    ...
    if vnext_foreign_key_count < 81 or not REQUIRED_VNEXT_FOREIGN_KEYS.issubset(
        vnext_foreign_keys
    ):
...
def apply(
    ...
    *,
    release_version: str = "unconfigured",
    dry_run: bool = False,
) -> dict[str, object]:
    try:
...
def main() -> int:
    ...
    parser.add_argument(
        "--database-url",
        default=None,
        help="Explicit managed or disposable PostgreSQL URL; never read from environment.",
    )
    parser.add_argument("--release-version", default="unconfigured")
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()
    result = apply(
        args.database_url, release_version=args.release_version, dry_run=args.dry_run
    )
