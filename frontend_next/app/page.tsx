...
import { api, downloadTaxReport, MapNearbyResult, MapSearchResult, MarketResult, NearbyCategory, NearbyPlace, TaxCase, TaxResult } from "@/lib/api";
...
function MapInsight() {
  ...
  return <div className="space-y-4"><PageHeader kicker="區域洞察" title="Map Insight 周遭生活機能" description="搜尋地址，查看 800 公尺生活圈的交通、採買、餐飲與公共設施。" />
    {error && <ErrorState message={error} />}{result ? <><div className="overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-[0_14px_40px_rgba(71,85,105,0.12)] xl:relative xl:min-h-[650px]"><div className="relative h-[420px] min-w-0 sm:h-[500px] xl:absolute xl:inset-0 xl:h-auto"><GeoMap center={result.center} zoom={15} categories={categories} selectedPlace={selectedPlace} onSelectPlace={setSelectedPlace} />
      ...
      <aside className="min-w-0 border-t border-stone-200 bg-white p-4 xl:absolute xl:bottom-4 xl:right-4 xl:top-20 xl:z-[500] xl:w-[340px] xl:overflow-y-auto xl:rounded-xl xl:border xl:border-white/80 xl:bg-white/92 xl:shadow-lg xl:backdrop-blur-md"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><p className="text-[10px] font-bold tracking-wider text-cyan-700">AREA INSIGHT</p><SourceBadge source={result.source} /></div><h2 className="mt-1 text-sm font-bold">{location?.district || location?.road || "區域總覽"}</h2></div><div className="text-right"><p className="text-3xl font-bold">{result.livability_score}</p><p className="text-[9px] text-slate-400">生活機能</p></div></div><p className="mt-3 break-words text-[11px] leading-5 text-slate-500">{result.summary}</p><h3 className="mt-4 text-xs font-bold">設施分類</h3><div className="mt-2 flex flex-wrap gap-2"><button onClick={() => setActive(allSelected ? [] : categoryKeys)} className={`rounded-full border px-2.5 py-1.5 text-[10px] font-bold ${allSelected ? "border-slate-700 bg-slate-800 text-white" : "border-stone-200 bg-white text-slate-500"}`}>全部</button>{result.categories.map((group) => <button key={group.category} onClick={() => setActive((items) => items.includes(group.category) ? items.filter((x) => x !== group.category) : [...items, group.category])} className={`flex items-center gap-1.5 rounded-full border px-2.5 py-1.5 text-[10px] font-bold ${active.includes(group.category) ? "border-cyan-300 bg-cyan-50 text-cyan-800" : "border-stone-200 bg-white text-slate-400"}`}><span>{group.label}</span><span className="text-[9px] opacity-60">{group.count}</span></button>)}</div><h3 className="mt-4 text-xs font-bold">附近地點</h3><PlaceList categories={categories} selected={selectedPlace} onSelect={setSelectedPlace} /><p className="mt-4 break-words border-t border-stone-200 pt-3 text-[10px] leading-5 text-amber-700">{result.source === "mock" ? "目前使用展示資料。" : ""} {result.disclaimer}</p></aside>
    </div></> : <LoadingState label="載入周遭生活機能中..." />}</div>;
}

function SourceBadge({ source }: { source: string }) {
  return <span className={`rounded-full px-2 py-0.5 text-[8px] font-bold ${source === "google_places" ? "bg-blue-100 text-blue-700" : "bg-amber-100 text-amber-700"}`}>{source === "google_places" ? "Google Places" : "Mock fallback"}</span>;
}
...
function PlaceList({ categories, selected, onSelect }: { categories: NearbyCategory[]; selected?: NearbyPlace; onSelect: (place: NearbyPlace) => void }) {
  ...
  return <div className="mt-2 space-y-2">{places.length ? places.map((place) => <button key={place.place_id} onClick={() => onSelect(place)} className={`w-full rounded-lg border p-2.5 text-left transition ${selected?.place_id === place.place_id ? "border-cyan-400 bg-cyan-50" : "border-stone-200 bg-white hover:border-cyan-200"}`}><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-xs font-bold text-slate-800">{place.name}</p><p className="mt-0.5 text-[9px] font-bold text-cyan-700">{place.label} · {place.distance_m} 公尺</p></div><span className="shrink-0 text-[9px] text-slate-400">{place.business_status === "OPERATIONAL" ? "營業中" : place.business_status}</span></div><p className="mt-1 truncate text-[9px] text-slate-400">{place.address}</p>{place.rating && <p className="mt-1 text-[9px] font-bold text-amber-600">★ {place.rating} ({place.user_rating_count})</p>}</button>) : <p className="rounded-lg bg-stone-50 p-3 text-[10px] text-slate-400">目前分類沒有找到周遭設施。</p>}</div>;
}
