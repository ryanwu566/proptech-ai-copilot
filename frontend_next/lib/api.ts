...
export type ValuationStatus = "available" | "no_data" | "unavailable" | "demo";
export type ValuationResultOrigin = "official" | "demo" | "none";
export type ValuationContractMetadata = { valuation_status: ValuationStatus; valuation_reason_code: string; result_origin: ValuationResultOrigin; is_actionable: boolean };
export type ValuationNullableMetrics = { estimate_total_price: number | null; estimate_unit_price_per_ping: number | null; price_range: { low: number | null; mid: number | null; high: number | null }; confidence_score: number | null };
export type ValuationPublicSourceDetails = { provider_active?: string; candidate_pool_size?: number; query_scope?: string; requested_city?: string; requested_district?: string; requested_road?: string; db_rows_returned?: number; query_status?: string };
export type ValuationTrendContractMetadata = { trend_status: "available" | "no_data" | "unavailable"; trend_reason_code: string; is_actionable: boolean };
export type PropertySearchContractMetadata = { search_status: "available" | "no_data" | "unavailable"; search_reason_code: string; is_actionable: boolean };
export type ValuationDataStatus = { active_source: "real_price_sample" | "sqlite_index" | "postgres" | "mock_fallback" | "unknown"; is_demo_data: boolean; is_full_taiwan: boolean; data_composition?: "sample" | "official" | "mixed"; official_records_count?: number; sample_records_count?: number; official_period_min?: string | null; official_period_max?: string | null; raw_official_period_min?: string | null; raw_official_period_max?: string | null; effective_trend_period_min?: string | null; effective_trend_period_max?: string | null; excluded_future_period_count?: number; excluded_too_old_period_count?: number; data_quality_note?: string; retention_policy_years?: number; retention_cutoff_period?: string | null; records_outside_retention_count?: number; oldest_effective_period?: string | null; newest_effective_period?: string | null; retention_note?: string; official_coverage_note?: string; coverage_city_count?: number; coverage_district_count?: number; coverage_road_count?: number; coverage_cities?: string[]; coverage_summary?: string; coverage_note_short?: string; latest_import_status?: string | null; latest_import_scope?: string; latest_import_inserted_rows?: number; latest_import_skipped_duplicates?: number; coverage: { cities: string[]; districts: string[]; roads_count: number; records_count: number }; last_updated: string | null; update_frequency_note: string; source_note: string; user_message: string; freshness_status: ValuationFreshnessStatus; freshness_reason_code: string; freshness_as_of: string | null; latest_import_at: string | null; latest_import_age_days: number | null; newest_effective_period_lag_months: number | null; operator_attention_required: boolean; freshness_user_message: string };
...
export const api = {
  ...
  propertySearch: (payload: Record<string, string | number | string[] | undefined>, signal?: AbortSignal) => request<PropertySearchResult>("/valuation/property-search", { method: "POST", body: JSON.stringify(payload), signal }),
