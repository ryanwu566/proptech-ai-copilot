...
export function PropertyCaseReadiness({ draft }: { draft: PropertyCaseDraft }) {
  ...
  const statuses = [
    ...
    ["稅務", draft.analysis_status.tax],
  ] as const;

  return <section className="rounded-xl border border-stone-200 bg-white p-4" aria-label="案件決策狀態">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div>
        <p className="text-[10px] font-bold tracking-wider text-cyan-700">PROPERTY CASE</p>
        <h3 className="mt-1 text-base font-bold text-slate-950">案件決策狀態</h3>
    ...
    <div className="mt-4 grid gap-2 text-[11px] sm:grid-cols-2 xl:grid-cols-4">
      <InfoPill label="案件名稱" value={draft.case_name} />
      <InfoPill label="物件地址" value={draft.property_input.address || "尚未輸入"} />
      <InfoPill label="參考價格" value={draft.property_input.listing_price ? `${draft.property_input.listing_price.toLocaleString()} 萬` : "尚未完成"} />
    ...
    <div className="mt-4 grid gap-3 md:grid-cols-2">
      ...
      <div className="rounded-lg border border-amber-100 bg-amber-50/60 p-3">
        ...
        <ul className="mt-2 space-y-1 text-[11px] leading-5 text-amber-900">
          {(readiness.safeWarnings.length ? readiness.safeWarnings : ["案件狀態只整理既有分析，不新增購買建議或安全保證。"]).map((warning) => <li key={warning}>• {warning}</li>)}
        </ul>
      </div>
    </div>
    {draft.readiness.missing_required.length > 0 && <p className="mt-3 text-[10px] leading-5 text-slate-500">缺少資料：{draft.readiness.missing_required.map(moduleLabel).join("、")}</p>}
  </section>;
}
