...
export function PropertyFinder({ onUseForValuation, onUseForLoan, onUseForHoldingCost, onUseForLocationInsight, onResult, initialResult }: { onUseForValuation: (selection: PropertyFinderSelection) => void; onUseForLoan: (priceWan: number) => void; onUseForHoldingCost: (priceWan: number, areaPing: number) => void; onUseForLocationInsight: (selection: PropertyFinderSelection, priceWan: number) => void; onResult?: (result: PropertySearchResult) => void; initialResult?: PropertySearchResult }) {
  ...
  return <div className="min-w-0 space-y-8"><div id="property-finder" className="scroll-mt-20"><SectionCard title="找房雷達" description="依預算與條件，從官方實價登錄成交資料找出較符合的區域與路段，作為看屋方向參考。">
...
function FinderActions({ onValuation, onLoan, onHoldingCost, onLocation }: { onValuation: () => void; onLoan: () => void; onHoldingCost: () => void; onLocation?: () => void }) {
  return <div className="grid min-w-[190px] grid-cols-2 gap-1.5 sm:flex sm:flex-wrap"><button type="button" aria-label="帶入估價" onClick={onValuation} className="whitespace-nowrap rounded-md border border-cyan-200 bg-white px-2.5 py-1.5 font-bold text-cyan-800 transition hover:bg-cyan-50">用這個路段估價</button><button type="button" aria-label="帶入貸款" onClick={onLoan} className="whitespace-nowrap rounded-md border border-violet-200 bg-white px-2.5 py-1.5 font-bold text-violet-800 transition hover:bg-violet-50">用這個價格算月付</button><button type="button" aria-label="帶入持有成本" onClick={onHoldingCost} className="whitespace-nowrap rounded-md border border-amber-200 bg-white px-2.5 py-1.5 font-bold text-amber-800 transition hover:bg-amber-50">算每月總支出</button><button type="button" aria-label="分析區位" onClick={onLocation} className="whitespace-nowrap rounded-md border border-emerald-200 bg-white px-2.5 py-1.5 font-bold text-emerald-800 transition hover:bg-emerald-50">看附近生活機能</button></div>;
}
