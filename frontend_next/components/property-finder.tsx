...
export function PropertyFinder({ onUseForValuation, onUseForLoan, onUseForHoldingCost, onUseForLocationInsight, onUseForMarketInsight, onResult, initialResult, embedded = false }: { onUseForValuation: (selection: PropertyFinderSelection) => void; onUseForLoan: (priceWan: number, selection: PropertyFinderSelection) => void; onUseForHoldingCost: (priceWan: number, areaPing: number, selection: PropertyFinderSelection) => void; onUseForLocationInsight: (selection: PropertyFinderSelection, priceWan: number) => void; onUseForMarketInsight?: (selection: PropertyFinderSelection) => void; onResult?: (result: PropertySearchResult | undefined) => void; initialResult?: PropertySearchResult; embedded?: boolean }) {
  ...
  return <div className="min-w-0 space-y-8"><div id="property-finder" className="scroll-mt-20"><SectionCard title={copy("finder.title")} description={copy("finder.description")}>
    ...
    {result && !loading && <PropertyFinderResults result={result} onUseForValuation={onUseForValuation} onUseForLoan={onUseForLoan} onUseForHoldingCost={onUseForHoldingCost} onUseForLocationInsight={onUseForLocationInsight} onUseForMarketInsight={onUseForMarketInsight} />}
  </SectionCard></div>{!embedded && <ImmersiveViewingWorkspace propertySearch={result}/>}</div>;
}

function PropertyFinderResults({ result, onUseForValuation, onUseForLoan, onUseForHoldingCost, onUseForLocationInsight, onUseForMarketInsight }: { result: PropertySearchResult; onUseForValuation: (selection: PropertyFinderSelection) => void; onUseForLoan: (priceWan: number, selection: PropertyFinderSelection) => void; onUseForHoldingCost: (priceWan: number, areaPing: number, selection: PropertyFinderSelection) => void; onUseForLocationInsight: (selection: PropertyFinderSelection, priceWan: number) => void; onUseForMarketInsight?: (selection: PropertyFinderSelection) => void }) {
...
function FinderActions({ onValuation, onLoan, onHoldingCost, onLocation, onMarket }: { onValuation: () => void; onLoan: () => void; onHoldingCost: () => void; onLocation?: () => void; onMarket?: () => void }) {
  const { copy } = useExperienceLocale();
  return <div className="grid min-w-[190px] grid-cols-2 gap-1.5 sm:flex sm:flex-wrap"><button type="button" aria-label={copy("finder.useValuation")} onClick={onValuation} className="whitespace-nowrap rounded-md border border-cyan-200 bg-white px-2.5 py-1.5 font-bold text-cyan-800 transition hover:bg-cyan-50">{copy("finder.useValuation")}</button><button type="button" aria-label={copy("finder.useLoan")} onClick={onLoan} className="whitespace-nowrap rounded-md border border-violet-200 bg-white px-2.5 py-1.5 font-bold text-violet-800 transition hover:bg-violet-50">{copy("finder.useLoan")}</button><button type="button" aria-label={copy("finder.useHolding")} onClick={onHoldingCost} className="whitespace-nowrap rounded-md border border-amber-200 bg-white px-2.5 py-1.5 font-bold text-amber-800 transition hover:bg-amber-50">{copy("finder.useHolding")}</button><button type="button" aria-label={copy("finder.useLocation")} onClick={onLocation} className="whitespace-nowrap rounded-md border border-emerald-200 bg-white px-2.5 py-1.5 font-bold text-emerald-800 transition hover:bg-emerald-50">{copy("finder.useLocation")}</button>{onMarket && <button type="button" aria-label={copy("finder.useMarket")} onClick={onMarket} className="whitespace-nowrap rounded-md border border-blue-200 bg-white px-2.5 py-1.5 font-bold text-blue-800 transition hover:bg-blue-50">{copy("finder.useMarket")}</button>}</div>;
}
