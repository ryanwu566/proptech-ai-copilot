...
function FinderActions({ onValuation, onLoan, onHoldingCost }: { onValuation: () => void; onLoan: () => void; onHoldingCost: () => void }) {
  return <div className="flex gap-1"><button type="button" onClick={onValuation} className="whitespace-nowrap rounded-lg border border-cyan-200 px-2 py-1 font-bold text-cyan-800 transition hover:bg-cyan-50">帶入估價</button><button type="button" onClick={onLoan} className="whitespace-nowrap rounded-lg border border-violet-200 px-2 py-1 font-bold text-violet-800 transition hover:bg-violet-50">帶入貸款</button><button type="button" onClick={onHoldingCost} className="whitespace-nowrap rounded-lg border border-amber-200 px-2 py-1 font-bold text-amber-800 transition hover:bg-amber-50">帶入持有成本</button></div>;
}
