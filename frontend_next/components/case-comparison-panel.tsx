...
export function CaseComparisonPanel({ savedCases, selectedIds }: { savedCases: SavedCase[]; selectedIds: string[] }) {
  const selected = savedCases.filter((item) => selectedIds.includes(item.id)).slice(0, 4);
  ...
  if (selected.length < 2) return <div className="mt-4 rounded-xl border border-dashed border-cyan-200 bg-cyan-50 p-4 text-xs text-cyan-900">請至少選擇兩個案件，最多可比較四個案件。</div>;
  return <section className="mt-4 min-w-0 space-y-4 rounded-xl border border-cyan-200 bg-cyan-50/40 p-4" aria-label="案件比較與候選排序">
    ...
    <DetailDisclosure title="查看案件比較完整表"><div className="max-w-full touch-pan-x overflow-x-auto"><table className="w-full min-w-[920px] text-left text-[10px]"><thead><tr className="bg-white"><th className="p-2">案件</th><th>總價</th><th>估價</th><th>月付</th><th>持有成本</th><th>區位</th><th>風險</th><th>稅務</th><th>完成度</th></tr></thead><tbody>{result.cases.map((item) => <tr key={item.caseId} className="border-t border-cyan-100 bg-white/80"><td className="p-2 font-bold">{item.title}</td><td>{formatWan(item.propertyPrice)}</td><td>{formatWan(item.valuationMid)}</td><td>{formatYuan(item.monthlyPayment)}</td><td>{formatYuan(item.monthlyHoldingCost)}</td><td>{item.locationScore ?? "尚未完成"}</td><td>{item.riskSignal} / {item.riskScore ?? "—"}</td><td>{item.taxStatus}</td><td>{item.completionRate}%</td></tr>)}</tbody></table></div></DetailDisclosure>
...
const detailRows: Array<[string, (item: ReturnType<typeof compareSavedCases>["cases"][number]) => string | number]> = [
  ...
  ["區位風險資料缺口", (item) => item.locationRiskGap], ["主要風險", (item) => item.mainRisks.join("、") || "尚未完成"], ["主要加分", (item) => item.positives.join("、") || "尚未完成"],
