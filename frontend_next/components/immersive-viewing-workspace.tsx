...
export function ImmersiveViewingWorkspace({ propertySearch }: { propertySearch?: PropertySearchResult }) {
  ...
  const [caseMessage, setCaseMessage] = useState("可以先保存案件，之後繼續分析。");
  useEffect(() => {
    ...
    const clearedListener = () => { setContext({ inputs: { city: "", district: "", road: "", building_type: "", area_ping: 0, building_age_years: 0, floor: 0 } }); setHoldingResult(undefined); setLocation(undefined); setWorkflowSession({ reportCompleted: false }); setCaseMessage("可以先保存案件，之後繼續分析。"); };
