...
function MapSummary({ city, district, road, areaPing, buildingType, valuation, location, onExport }: { city: string; district: string; road: string; areaPing: number; buildingType: string; valuation?: ValuationResult; location?: LocationInsightResult; onExport: () => void }) {
  return <div className="overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm"><div className="relative h-28 bg-[radial-gradient(circle_at_25%_30%,#67e8f9_0_4px,transparent_5px),radial-gradient(circle_at_70%_55%,#fbbf24_0_4px,transparent_5px),linear-gradient(135deg,#e2e8f0,#f8fafc)]"><div className="absolute inset-3 rounded-lg border border-white/80 bg-white/35" /><span className="absolute bottom-2 right-3 text-[9px] font-bold text-slate-500">區位地圖摘要卡</span></div><div className="p-4"><p className="text-[10px] font-bold text-cyan-700">目前地點</p><h3 className="mt-1 font-bold text-slate-950">{location?.resolved_location?.address_label || `${city}${district}${road}`}</h3><p className="mt-1 text-[10px] text-slate-500">{areaPing} 坪 · {buildingType} · {valuation ? `${valuation.price_range.mid.toLocaleString()} 萬` : "尚未估價"}</p><div className="mt-3 grid grid-cols-2 gap-2 text-center"><MiniMetric label="區位分數" value={location?.location_score ?? "—"} /><MiniMetric label="交通 POI" value={location?.poi_summary.transit_count ?? "—"} /><MiniMetric label="便利 POI" value={location?.poi_summary.convenience_count ?? "—"} /><MiniMetric label="資料品質" value={location?.data_quality.status ?? "未分析"} /></div><div className="mt-3 space-y-2"><Button secondary className="w-full" onClick={() => prefillLocationInsight({ city, district, road, area_ping: areaPing, building_type: buildingType, property_price: valuation?.price_range.mid })}>分析區位</Button><Button className="w-full" disabled={!valuation} onClick={onExport}>匯出看屋報告</Button></div>{location?.nearest_pois.length ? <ul className="mt-3 space-y-1 text-[10px] text-slate-500">{location.nearest_pois.slice(0, 3).map((item) => <li key={`${item.name}-${item.distance_m}`}>{item.name} · {item.distance_m}m</li>)}</ul> : <p className="mt-3 text-[10px] leading-5 text-amber-700">完成區位分析後，這裡會顯示最近 POI 與資料限制。</p>}</div></div>;
}
