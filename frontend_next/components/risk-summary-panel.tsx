...
export function RiskSummaryPanel({ summary }: { summary: RiskSummary }) {
  ...
  return <section id="risk-summary" className="min-w-0 scroll-mt-20 overflow-hidden rounded-xl border border-stone-200 bg-white" aria-label={copy("risk.heading")}>
    <div className={`border-b p-4 ${tones[summary.overallSignal]}`}>
      <div className="flex flex-wrap items-center gap-3"><span className={`h-12 w-12 shrink-0 rounded-full border-4 border-white shadow-md ${lights[summary.overallSignal]}`} /><div className="min-w-0 flex-1"><p className="text-[10px] font-bold tracking-wider">{copy("risk.kicker")}</p><h2 className="mt-1 text-lg font-extrabold">{copy("risk.heading")}: {summary.overallLabel}</h2><p className="mt-1 text-xs leading-5">{summary.decisionSuggestion}</p></div><div className="rounded-lg bg-white/75 px-3 py-2 text-center"><p className="text-[10px]">{copy("risk.overallScore")}</p><p className="text-xl font-black">{summary.overallScore ?? "—"}</p></div></div>
    </div>
    <div className="p-4"><DetailDisclosure title={copy("risk.sourceTitle")}><div className="grid min-w-0 gap-3 md:grid-cols-2">
      <SummaryBlock title={copy("risk.priceReasonableness")} items={[`${summary.priceReasonableness.label}: ${summary.priceReasonableness.explanation}`]} noEmpty={copy("risk.noPositive")} />
      <SummaryBlock title={copy("risk.dataConfidence")} items={[confidenceLabelFn(summary.dataConfidence)]} noEmpty={copy("risk.noPositive")} />
      <SummaryBlock title={copy("risk.positiveFactors")} items={summary.positiveFactors.map((item) => `${item.title}: ${item.message}`)} noEmpty={copy("risk.noPositive")} />
      <SummaryBlock title={copy("risk.riskFactors")} items={summary.riskFactors.map((item) => `${item.title}: ${item.message}`)} noEmpty={copy("risk.noRisk")} />
      <SummaryBlock title={copy("risk.missingChecks")} items={summary.missingChecks} noEmpty={copy("risk.noMissing")} />
      <SummaryBlock title={copy("risk.nextSteps")} items={summary.nextActions} noEmpty={copy("risk.noNextSteps")} />
