...
import { localizeRiskSignalLabel, localizeRiskSuggestion, localizePriceLabel, localizeFactorTitle, type RiskSignal } from "@/lib/dynamic-copy-localizers";
...
export function RiskSummaryPanel({ summary }: { summary: RiskSummary }) {
  ...
  return <section id="risk-summary" className="min-w-0 scroll-mt-20 overflow-hidden rounded-xl border border-stone-200 bg-white" aria-label={copy("risk.heading")}>
    ...
    <div className="p-4"><DetailDisclosure title={copy("risk.sourceTitle")}><div className="grid min-w-0 gap-3 md:grid-cols-2">
      <SummaryBlock title={copy("risk.priceReasonableness")} items={[`${localizedPriceLabel}: ${summary.priceReasonableness.explanation}`]} noEmpty={copy("risk.noPositive")} />
      <SummaryBlock title={copy("risk.dataConfidence")} items={[confidenceLabelFn(summary.dataConfidence)]} noEmpty={copy("risk.noPositive")} />
      <SummaryBlock title={copy("risk.positiveFactors")} items={summary.positiveFactors.map((item) => `${localizeFactorTitle(item.key, locale)}: ${item.message}`)} noEmpty={copy("risk.noPositive")} />
      <SummaryBlock title={copy("risk.riskFactors")} items={summary.riskFactors.map((item) => `${localizeFactorTitle(item.key, locale)}: ${item.message}`)} noEmpty={copy("risk.noRisk")} />
