...
import { getJourneyStepCopy, type JourneyStepDefinition, type JourneyStepId } from "@/lib/guided-journey";
import { useExperienceLocale } from "@/components/experience-locale-provider";

type JourneyStepperProps = { steps: readonly JourneyStepDefinition[]; activeStep: JourneyStepId; visitedSteps: readonly JourneyStepId[]; onSelect: (step: JourneyStepId) => void };
...
function StepButton({ step, activeStep, visitedSteps, onSelect }: StepButtonProps) {
  ...
  const status = active ? t("journey.statusCurrent") : visited ? t("journey.statusVisited") : t("journey.statusNotVisited");
  return <button type="button" aria-current={active ? "step" : undefined} aria-label={`${copy.title} · ${status}`} onClick={() => onSelect(step.id)} className={`w-full rounded-xl border px-3 py-3 text-left transition focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 ${active ? "border-cyan-400 bg-cyan-50 shadow-sm" : "border-stone-200 bg-white hover:border-cyan-200"}`}>
    <span className="flex items-start gap-3"><span className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-xs font-black ${active ? "bg-cyan-700 text-white" : visited ? "bg-cyan-100 text-cyan-900" : "bg-stone-100 text-slate-500"}`}>{step.number}</span><span className="min-w-0"><strong className="block text-sm text-slate-900">{copy.title}</strong><span className="mt-1 block text-[11px] text-slate-500">{status}</span></span></span>
  </button>;
}

export function JourneyStepper({ steps, activeStep, visitedSteps, onSelect }: JourneyStepperProps) {
  const { t } = useExperienceLocale();
  return <nav aria-label={t("journey.mobileSummary")} className="space-y-3">
    ...
    <details className="rounded-xl border border-stone-200 bg-white lg:hidden">
      <summary className="cursor-pointer px-3 py-3 text-sm font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-inset">{t("journey.mobileSummary")}</summary>
