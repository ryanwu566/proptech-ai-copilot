...
export function PriceDecisionStage({ propertyContext, renderValuation, renderPropertySearch, onBackToLocation, onContinueToAffordability, onTransferToLoan, onTransferToHolding }: { propertyContext: JourneyPropertyContext; renderValuation: (context: JourneyPropertyContext, handlers: PriceHandlers) => ReactNode; renderPropertySearch: () => ReactNode; onBackToLocation: () => void; onContinueToAffordability: () => void; onTransferToLoan: (priceWan: number) => void; onTransferToHolding: (priceWan: number, areaPing?: number) => void }) {
  ...
  return <div className="min-w-0 space-y-4">
    ...
    <section aria-labelledby="price-decision-workspace-heading" className="min-w-0 space-y-3"><div><h3 id="price-decision-workspace-heading" className="text-lg font-black text-slate-950">{t("journey.price.title")}</h3><p className="mt-1 text-xs leading-5 text-slate-600">{t("journey.price.description")}</p></div>{renderValuation(propertyContext, { onResult: handleResult, onStatusChange: setStatusOverride })}</section>
    ...
    <details className="rounded-xl border border-stone-200 bg-white"><summary className="cursor-pointer px-4 py-3 text-sm font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-cyan-500">{t("evidence.details")}</summary><div className="border-t border-stone-100 p-4">{renderPropertySearch()}</div></details>
