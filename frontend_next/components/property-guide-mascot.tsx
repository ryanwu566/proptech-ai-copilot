...
export function PropertyGuideMascot({ stage }: { stage: "start" | "finder" | "valuation" | "loan" | "location" | "complete" }) {
  ...
  return <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 p-3" aria-label="黃色看房助手">
    ...
    <div><p className="text-[10px] font-bold tracking-wider text-amber-700">黃色看房助手</p><p className="mt-1 text-xs leading-5 text-slate-700">{messages[stage]}</p></div>
  </div>;
}
