    ...
    <div className="relative grid min-w-0 gap-7 xl:grid-cols-[minmax(0,1fr)_minmax(420px,0.95fr)] xl:items-center">
      <div className="min-w-0 hero-reveal motion-reduce:animate-none">
        ...
        <div className="mt-6 grid gap-2 sm:flex sm:flex-wrap">
          ...
          <button type="button" data-action-kind="secondary" onClick={onWorkspace} className="rounded-xl border border-white/25 bg-white/10 px-5 py-3 text-sm font-bold text-white transition hover:bg-white/15 focus:outline-none focus:ring-2 focus:ring-white/40 motion-reduce:transition-none">查看目前分析進度</button>
          <button type="button" data-action-kind="secondary" disabled={!reportReady} onClick={onReport} title={reportReady ? "查看看屋報告" : "完成估價後即可查看報告"} className="rounded-xl border border-white/15 px-5 py-3 text-sm font-bold text-slate-300 transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-45 motion-reduce:transition-none">查看看屋報告</button>
        </div>
      </div>
