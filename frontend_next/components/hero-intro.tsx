...
const flowNodes = ["找房雷達", "合理價格區間", "每月要繳多少", "房貸之外還要花多少", "附近生活機能", "值不值得繼續看", "可分享報告"];
const outcomeCards = [
  ...
  return <section id="hero" className="relative min-w-0 overflow-hidden rounded-3xl border border-cyan-200/70 bg-slate-950 px-4 py-6 text-white shadow-xl sm:px-7 sm:py-8 lg:px-10 lg:py-10">
    ...
    <div className="relative grid min-w-0 gap-7 xl:grid-cols-[minmax(0,1fr)_minmax(420px,0.95fr)] xl:items-center">
      <div className="min-w-0 hero-reveal motion-reduce:animate-none">
        ...
        <p className="mt-3 max-w-3xl rounded-xl border border-cyan-300/20 bg-cyan-300/10 px-3 py-2 text-xs leading-6 text-cyan-50">最後你會拿到：合理價格、月付壓力、區位優缺點、紅黃綠風險燈號，以及可分享的 HTML 報告。</p>
      ...
      <div className="relative min-w-0 rounded-2xl border border-white/10 bg-white/[0.06] p-4 backdrop-blur-sm">
        <div className="mb-4 flex items-center justify-between gap-3"><div><p className="text-[10px] font-bold tracking-wider text-cyan-200">你會完成的分析</p><p className="mt-1 text-sm font-bold">專業能力放在流程裡，不用一次看懂</p></div><GuideMark /></div>
