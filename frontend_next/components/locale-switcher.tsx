...
export function LocaleSwitcher() {
  ...
  return <label className="flex items-center gap-1.5 text-[10px] text-slate-600">
    ...
    <select value={locale} onChange={(event) => setLocale(event.target.value)} aria-label={t("locale.switcherLabel")} className="rounded-md border border-stone-200 bg-white px-2 py-1.5 font-bold focus:outline-none focus:ring-2 focus:ring-cyan-500">
