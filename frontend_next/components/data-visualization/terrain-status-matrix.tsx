...
export function TerrainStatusMatrix({ result }: { result: TerrainRiskResult | null }) {
  ...
  return <section aria-labelledby="terrain-status-matrix-heading" className="rounded-xl border border-amber-200 bg-amber-50/40 p-4">
    <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between"><h3 id="terrain-status-matrix-heading" className="text-sm font-black text-slate-950">風險資料來源與限制</h3><span className="text-[10px] text-amber-800">各圖層獨立呈現，不形成總體結論</span></div>
...
function TerrainRow({ row }: { row: { label: string; status: string; detail: string; source?: string } }) {
  const warning = ["部分可用", "涵蓋有限", "暫時不可用", "檢查失敗", "未知", "未評估"].includes(row.status);
