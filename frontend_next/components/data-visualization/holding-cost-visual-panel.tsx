...
export function HoldingCostVisualPanel({ model, result }: { model: HoldingCostVisualModel; result: HoldingCostResult }) {
  ...
  return <div className="min-w-0 space-y-4"><SectionCard title={copy.resultTitle}><p className="text-sm leading-6 text-slate-700">{model.summary}</p><div className="mt-3"><AffordabilityStatus value={model.affordability} /></div></SectionCard><div className="grid gap-3 sm:grid-cols-2"><MetricTile label={copy.monthlyTotal} value={`${model.metrics.monthlyTotal?.toLocaleString()} ${copy.currency}`} /><MetricTile label={copy.annualTotal} value={`${model.metrics.annualTotal?.toLocaleString()} ${copy.currency}`} /><MetricTile label={copy.incomeBurden} value={model.metrics.incomeBurden === null ? copy.unavailable : `${(model.metrics.incomeBurden * 100).toFixed(1)}%`} /><MetricTile label={copy.annualTax} value={model.metrics.annualTaxEstimate === null ? copy.unavailable : `${model.metrics.annualTaxEstimate.toLocaleString()} ${copy.currency}`} /></div><HoldingCostBreakdownChart model={model} /><SectionCard title={copy.breakdownTitle}><ul className="grid gap-2 text-xs text-slate-700 sm:grid-cols-2">{model.breakdown.slice(0, 4).map((item) => <li key={item.key} className="rounded-lg bg-stone-50 px-3 py-2.5"><strong>{item.label}</strong>: {item.monthlyAmount.toLocaleString()} {copy.currency}</li>)}</ul></SectionCard><DetailDisclosure title={copy.detailsTitle}><div className="space-y-3 text-xs text-slate-700"><dl className="grid gap-2 sm:grid-cols-2">{model.evidence.map((item) => <div key={item.key}><dt className="font-bold text-slate-800">{item.label}</dt><dd>{item.value}</dd></div>)}</dl><div className="max-w-full overflow-x-auto"><table className="w-full min-w-[520px] text-left"><thead><tr className="bg-stone-50"><th className="p-2">{copy.item}</th><th>{copy.monthly}</th><th>{copy.percentage}</th></tr></thead><tbody>{model.breakdown.map((item) => <tr key={item.key} className="border-t border-stone-100"><td className="p-2">{item.label}</td><td>{item.monthlyAmount.toLocaleString()} {copy.currency}</td><td>{item.percentage === null ? copy.unavailable : `${item.percentage.toFixed(1)}%`}</td></tr>)}</tbody></table></div><p>{copy.limitation}</p><p>{result.disclaimer}</p></div></DetailDisclosure></div>;
}
