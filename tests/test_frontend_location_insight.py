...
def test_location_insight_form_and_api_client_exist() -> None:
    ...
    for key in ("location.title", "location.address", "location.radius", "location.propertyPrice", "location.start"):
...
def test_location_results_and_unavailable_state_are_visible() -> None:
    for key in ("location.score", "location.transit", "location.convenience", "location.poiDetails", "location.dataQuality"):
        assert f'copy("{key}")' in COMPONENT
    assert 'copy("location.noResult")' in COMPONENT
...
def test_property_finder_and_valuation_can_prefill_location() -> None:
    assert 'copy("finder.useLocation")' in FINDER
