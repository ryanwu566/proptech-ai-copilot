...
def test_loan_calculator_form_and_api_client_exist() -> None:
    ...
    for key in ("loan.title", "loan.propertyPrice", "loan.downPayment", "loan.rate", "loan.years", "loan.grace", "loan.income", "loan.calculate"):
...
def test_valuation_and_property_finder_can_fill_loan_without_auto_submit() -> None:
    ...
    assert 'copy("valuation.useMedian")' in PAGE
    assert "result.price_range.mid" in PAGE
    assert 'copy("finder.useLoan")' in FINDER
