...
def test_decision_stage_controls_have_native_semantics() -> None:
    ...
    assert 'aria-labelledby="journey-decision-context-heading"' in context
    assert 'aria-labelledby="decision-case-action-heading"' in action
    assert "aria-pressed={active}" in action
...
def test_attention_and_saved_case_sections_are_textual_and_mobile_safe() -> None:
    ...
    for category in ("blocked", "missing", "unknown"):
        assert category in attention
    assert "grid gap-2 md:grid-cols-2" in attention
    assert 'aria-label={`${copy("case.compare")} ${saved.title}`}' in saved
