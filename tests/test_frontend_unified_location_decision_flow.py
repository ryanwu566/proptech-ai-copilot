...
def test_location_flow_has_single_clear_sequence() -> None:
    for key in ("location.title", "location.address", "location.start", "commute.title"):
        assert f'copy("{key}")' in LOCATION
...
def test_location_flow_has_one_primary_address_input() -> None:
    assert LOCATION.count('copy("location.address")') >= 2
    assert "setAddress(detail.address" in LOCATION
...
def test_blank_address_blocks_location_api_and_shows_message() -> None:
    ...
    assert 'copy("location.empty")' in LOCATION
