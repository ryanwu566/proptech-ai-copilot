...
def test_missing_data_is_not_rendered_as_zero_or_success() -> None:
    ...
    assert "價格資料待補與限制" in (STAGE_DIR / "price-decision-stage.tsx").read_text(encoding="utf-8")
    assert "資金與稅務待補資料" in (STAGE_DIR / "affordability-decision-stage.tsx").read_text(encoding="utf-8")
    assert "資料待補" in snapshot
