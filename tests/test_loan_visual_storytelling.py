...
def test_loan_form_stays_visible_and_existing_manual_transfer_is_preserved() -> None:
    assert 'copy("loan.title")' in COMPONENT
    assert 'copy("loan.calculate")' in COMPONENT
...
def test_loan_visual_hierarchy_and_safe_language_exist() -> None:
    ...
    for name in ("LoanFinancingStructureChart", "LoanSensitivityChart", "CalculationEvidenceDetails"):
        assert name in panel or name in evidence
    assert "structureMatches" in HELPER
...
def test_loan_affordability_and_income_states_are_explicit() -> None:
    ...
    assert '"unavailable"' in HELPER or '"not_assessed"' in HELPER
    assert "monthly_income" in HELPER
