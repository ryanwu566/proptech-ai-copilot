...
| liquefaction | Yes | Yes, from returned polygons | Yes: official low/medium/high potential classes |
...
- **Liquefaction production query path:** Public regional GeoJSON access exists, but a bounded production-safe query path has not yet been proven using only published API parameters. The official API returns polygons with `area`, `classify`, and `分級`, and its OpenAPI lists `bbox`, but it requires `area` and `classify`; the current provider has coordinates without an authoritative area router. A bounded probe returned HTTP 200 only when using response-bounding `all=false`, which is not allowed by the published OpenAPI enum. Feature-level freshness is also absent: group metadata was updated 2024-08-08, the separate [dataset index](https://data.gov.tw/dataset/28708) was updated 2025-06-19 and lists production year 2020, while the response carries no update timestamp.
