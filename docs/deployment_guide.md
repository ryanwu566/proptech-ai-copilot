...
目前不需要設定 `VALUATION_DATABASE_URL`，系統會使用 sample provider。未來切換 Supabase/Postgres 時，只能在 Render backend 設定此變數；Vercel 不需要。若 Postgres provider 尚未啟用或資料庫不可用，系統會安全 fallback 至 SQLite、sample 或展示資料。
