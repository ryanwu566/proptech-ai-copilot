...
Workspace RLS requires one stable, server-verifiable principal ID. The owner must choose and
document one of these before persistent Stage 1 work:

1. Supabase Auth with `auth.users.id` as the principal;
2. another IdP mapped server-side into an application principal table;
3. server-only FastAPI authorization with no browser Data API, while still applying database
   RLS/role defense in depth.

Do not mix pilot participant/reviewer roles into workspace membership implicitly. Do not use
JWT `user_metadata`/`raw_user_meta_data` for authorization because it is user-editable. If
claims are used, authorization values belong in controlled app metadata and remain subject to
token-staleness handling; membership tables are the source of truth for sensitive operations.
...
Supabase `service_role`/secret key, if selected, is allowed only in trusted server/operator
environments for narrowly scoped administrative tasks that genuinely require it: migrations,
policy-reviewed ingestion, signed Storage operations or controlled user administration. It is
not the normal authorization mechanism and must be wrapped by application authorization.
...
- broad request handlers that trust it instead of checking membership/resource scope.
...
1. private bucket per sensitivity class or one private bucket with policy-enforced object
   prefixes; never public buckets for case/title/contact documents;
...
- chosen authentication/principal mapping is documented;
...
Current result: contract defined, implementation not approved. Authentication/Supabase choice
and the missing deep-workflow audit remain owner blockers.
